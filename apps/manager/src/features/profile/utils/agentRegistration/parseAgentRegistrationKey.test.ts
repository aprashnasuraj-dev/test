...
} from './parseAgentRegistrationKey'
...
describe('parseAgentRegistrationKey', () => {
  ...
  it('should parse a key with numeric agent ID', () => {
    const key =
      'agent-registration[0x0001000001018004a169fb4a3325136eb29fa0ceb6d2e539a432][167]'
    const result = parseAgentRegistrationKey(key)
  ...
  it('should return null for malformed keys', () => {
    ...
    // Missing hex prefix
    expect(
      parseAgentRegistrationKey('agent-registration[1234][5678]'),
    ).toBeNull()
...
describe('isAgentRegistrationKey', () => {
  it('should return true for agent-registration keys', () => {
    expect(isAgentRegistrationKey('agent-registration[0x1234][5678]')).toBe(
      true,
    )
