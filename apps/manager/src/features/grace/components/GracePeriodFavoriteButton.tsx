...
type GracePeriodFavoriteButtonProps = {
  readonly className: string
  readonly name: string
}

export const GracePeriodFavoriteButton = ({
  className,
  name,
}: GracePeriodFavoriteButtonProps) => {
  const { _ } = useLingui()
  ...
  const button = (
    <button
      ...
      aria-label={
        isFavorite ? _(msg`Remove from favorites`) : _(msg`Add to favorites`)
      }
    ...
    >
      ...
      <MSymbol
        className={isFavorite ? 'ms-fill ms-wght-300' : 'ms-wght-300'}
        symbol="favorite"
      />
    </button>
  )
