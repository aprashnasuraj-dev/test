...
// vitest's typed mock.calls is `(unknown)[][]` under noUncheckedIndexedAccess;
// narrow the nth call's args explicitly so assertions stay type-safe.
const callArgs = (
...
describe('verifyProxyContract', () => {
  ...
  it("hits the chain's API URL with verifyproxycontract and the proxy address", async () => {
    ...
    const url = new URL(callArgs(fetchMock, 0)[0])
    ...
    expect(callArgs(fetchMock, 0)[1]?.method).toBe('POST')
  })
  ...
  it('polls checkproxyverification after a successful submit', async () => {
    ...
    const pollUrl = new URL(callArgs(fetchMock, 1)[0])
